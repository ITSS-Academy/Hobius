...
export class UserEbooksService {
    ...
    constructor(@InjectRepository(UserEbook) private userEbooksRepository: Repository<UserEbook>) {
    }
    ...
    async findAllByUserId(userId: string) {
        try {
            return await this.userEbooksRepository.createQueryBuilder('userEbook')
                .leftJoinAndSelect('userEbook.ebook', 'ebook')
                .select(['userEbook.readingStatus', 'userEbook.purchaseDate', 'userEbook.lastReadDate', 'userEbook.lastPageRead', 'userEbook.currentPage', 'ebook.id', 'ebook.title'])
    ...
    async findOneByEbookIdAndUserId(ebookId: string, userId: string) {
        try {
            return await this.userEbooksRepository.createQueryBuilder('userEbook')
                ...
                .select(['userEbook', 'ebook.id', 'ebook.title', 'user.id', 'user.userName', 'user.avatarURL'])
                .where('userEbook.userId = :userId', {userId})
    ...
    async update(ebookId: string, userId: string, userEbook: UpdateUserEbookDto) {
        try {
            await this.userEbooksRepository.createQueryBuilder()
                .update(UserEbook)
                .set(userEbook)
    ...
    async read(ebookId: string, userId: string, updateUserEbookDto: UpdateUserEbookDto) {
        try {
    ...
    async finishReading(ebookId: string, userId: string, updateUserEbookDto: UpdateUserEbookDto) {
        try {
            // Step 1: Find the UserEbook
            let userEbook = await this.findOneByEbookIdAndUserId(ebookId, userId);
